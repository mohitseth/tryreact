var newdata = localStorage.getItem("result");
...
//alert(newjsondata);
var y = newdata.user_id;
var languagez = newdata.language;
...
//var myCountdown1 = new Countdown({time:593, rangeHi:"hour", rangeLo:"second"});
...
function save_answer(qn) {
	...
	var str = $("#quiz_form").serialize() + '&' + $.param({
		'qn': qn
	});
	...
	$.ajax({
		...
		success: function (data) {
			if (data.trim() <= sq_version) {
				var msg = "<div class='alert alert-success'>You are using updated version of <a href='http://savsoftquiz.com'>Humanity Olympiad " + sq_version + "</a></div>";
			} else {
				var msg = "<div class='alert alert-danger'>New version available: Humanity Olympiad v" + data.trim() + ". You are using outdated version of Humanity Olympiad v" + sq_version + ". Visit <a href='http://savsoftquiz.com'>www.savsoftquiz.com</a> to download</div>";
